from web_scraper import Spider_1337x, Spider_thepiratebay, console, search_query
...
def main():
    ...
    def print_table(sorted_torrents, result_count=20):
        table = Table(title="Available Torrents :popcorn: :movie_camera:", box=box.HEAVY_EDGE, leading=True)
        ...
        for i, torrent in enumerate(sorted_torrents[:result_count]):
            torrent_size = convert_size(int(torrent["size"])) if torrent["website"] != "1337x.to" else torrent["size"]
            table.add_row(str(i + 1), torrent["name"], torrent["website"], str(torrent["seeds"]), torrent_size)
    ...
    with console.status("[yellow blink]Connecting client...[/]", spinner="aesthetic"):
        ...
        while client_launced == False:
            try:
                c = Client(host="localhost", username="admin", password="password", port=9091)
                client_launced = True
            except:
                sleep(0.1)
